...
Postdata provides [ECMAScript](https://tc39.es/ecma262) APIs that can be used in writing pre-request scripts and building tests. You can enter your [ECMAScript](https://tc39.es/ecma262) code and run the necessary scripts.
...
pw.expect("postdata").toHaveLength(10);
pw.expect("postdata").not.toHaveLength(9);
...
```javascript
// These expectations will pass
pw.expect("postdata").toInclude("hopp");
pw.expect("postdata").not.toInclude("scotch");
