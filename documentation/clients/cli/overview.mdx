...
Postdata gives you multiple ways to interact with and configure your APIs. With the command-line interface (CLI) you can interact with the Postdata platform using a terminal, or through an automated system. This enables you to run API tests, manage automated API monitoring, and more.

This section contains a complete list of all Postdata CLI commands available, alongside their optional parameters for additional behavior. You can also find a complete list of configuration options to configure your APIs through Postdata.

<Info>Postdata CLI is currently in alpha stage. Report a bug by [opening a new issue](https://github.com/mamahuhu-io/postdata/issues/new/choose).</Info>

## Pre-requisites
Before installing the Postdata CLI, ensure your system meets the following requirements.
...
## Installing Postdata CLI

Once the dependencies are installed, install @postdata/cli from npm by running:
...
The `hopp test` command allows you to run tests against a Postdata collection file.
...
```bash
hopp test [-e <environment file>] [-d <delay_in_ms> ] <postdata collection file>
```

### Running Collections present on the API client

The `hopp test` command can also be used to run collections present in your API client on Postdata cloud or self-hosted platforms.
...
```bash
hopp test  [-e <environment id>] [-d <delay_i_ms>] <postdata collection id> [--token <access_token>] [--server <server url>]
```
...
- `postdata collection id` : Each collection created in a Postdata workspace is given a unique identifier known as the Collection ID. Collection IDs for each collection can be found under **“Details”** tab inside Collection **“Properties”**.
    
- `environment id` : Similar to Collection IDs, each environment created in a Postdata workspace is assigned a unique identifier known as the Environment ID.
- `delay_i_ms` : Represents a time interval (in milliseconds) to pause execution of API requests before within a collection.
- `access token` : It is a secure, unique identifier used to authenticate a user's access to their Postdata account and its resources like collections, environments data. [Learn more about personal access tokens](/documentation/features/pat)
...
- `no_of_iterations`: Indicates the number of iterations to run the collection. Each iteration will run the entire collection once, replacing any iteration-specific data defined by the `--iteration-data` flag (if provided).
  ...
  ```bash
  hopp test <postdata_collection_file_or_id> [--iteration-count <no_of_iterations>] [--iteration-data <file_path>]
  ```
...
```bash
hopp test kitchen-sink-postdata-collection.json
hopp test -e environment.json kitchen-sink-postdata-collection.json
hopp test -e environment.json -d 1000 kitchen-sink-postdata-collection.json
...
hopp test -e environment.json kitchen-sink-postdata-collection.json --reporter-junit kitchen-sink-junit.xml
hopp test kitchen-sink-postdata-collection.json --iteration-count 3 --iteration-data /path/to/iteration-data.csv
```

## Environment

Postdata allows templates in several places. For example, you could specify your endpoint URL as `<<baseurl>>/post` and specify baseurl as [`https://echo.postdata.cn`](https://echo.postdata.cn) in an environment file.

Postdata CLI supports environment files in two specific formats:

### 1. Single Environment Entry Export Format

This format is generated by Postdata App when you export any of your environment. It includes a named environment (name) with key-value pairs, allowing you to define various variables within a single file.
  ```json
  {
    "name": "my_env",
    "variables": [
...
Postdata CLI continues to support the legacy format which was previously the only accepted format used by CLI.
```json
{
  "key1": "value1",
  "key2": "value2",
...
To use an environment on your API client using its ID, click on the `Properties` action present in the **menu icon** next to each environment. Within the Details section, you'll find the **Environment ID**. Copy this ID and use it in the Postdata CLI for execution.

<Note>Please note that the Postdata CLI exclusively supports the above three formats for importing environment variables. It **does not** offer compatibility with **Bulk Environment** exports or **any other export** format.</Note>
