---
sidebarTitle: RESTful API testing with Postdata
title: RESTful API testing with Postdata
description: Learn how to use Postdata to test your RESTful APIs.
---

Representational State Transfer (REST) APIs are a core part of communication between a web/mobile client and a server. The majority of web-based applications depend on REST APIs to fetch and modify data thus separating the data processing part from the front-end. Postdata provides a fast and intuitive platform to develop and test REST APIs making it easier for developers worldwide to work efficiently.
...
REST APIs also support HTTP methods to do operations on data. Postdata supports the following methods out of the box.
...
Postdata provides you with a minimal yet robust platform to test your REST APIs. The REST platform is the default platform you see when you open Postdata.
...
```
https://echo.postdata.cn
```

### Locally served APIs

If you are planning to use Postdata to test your locally served APIs, it is recommended that you install the Postdata Browser Extension. Once installed, switch the interceptor on Postdata to Browser Extension from the settings page or bottom bar to add support for [localhost](http://localhost) protocols.
...
Postdata has support for various types of authentication mechanisms such as Basic Auth, Bearer Token, OAuth 2.0, and API Key. You can configure this using the Authorization tab on the request section of Postdata.

These are just a few of the amazing set of features that Postdata provides you to make your life easier as a developer. In addition to this, Postdata supports features such as collections to save your requests, pre-requests to add dynamic behavior to your requests and so much more.
