---
sidebarTitle: Understanding the GraphQL Schema on Postdata
title: Understanding the GraphQL Schema on Postdata
description: Learn how to use the GraphQL schema on Postdata.
---

The GraphQL schema is at the core of any GraphQL server. It defines the capabilities of the API and specifies how clients can access that data. A schema includes types and fields, queries and mutations, interfaces, and unions. Postdata provides you the functionality to connect to a GraphQL server and explore the GraphQL schema.
...
Postdata's GraphQL API platform provides you with the best experience to test and play around with GraphQL. Once connected to a GraphQL server, you can view the schema by clicking the schema icon on the sidebar.

For example, try connecting to the following server

```
https://echo.postdata.cn/graphql
```
